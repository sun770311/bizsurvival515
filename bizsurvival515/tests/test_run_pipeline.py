...
from bizsurvival515.pipeline.run_pipeline import main
...
class TestRunPipeline(unittest.TestCase):
    ...
    def _run_main_with_patches(
        ...
        with (
            patch("bizsurvival515.pipeline.run_pipeline.parse_args", return_value=args),
            patch(
                "bizsurvival515.pipeline.run_pipeline.run_preprocess_pipeline",
                return_value=context["joined_path"],
            ) as mock_preprocess,
            patch(
                "bizsurvival515.pipeline.run_pipeline.run_logistic_pipeline",
                return_value=context["logistic_results"],
            ) as mock_logistic,
            patch(
                "bizsurvival515.pipeline.run_pipeline.run_cox_full_pipeline",
                return_value=context["cox_results"],
            ) as mock_cox,
            patch(
                "bizsurvival515.pipeline.run_pipeline.run_geojson_pipeline",
    ...
    def test_main_raises_when_input_files_are_missing(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch("bizsurvival515.pipeline.run_pipeline.parse_args", return_value=args):
