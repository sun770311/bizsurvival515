...
This script loads the saved logistic regression pipeline and artifacts,
prints model diagnostics, checks whether hypothetical predictions fall on
the expected side of the baseline, and prints predictions for several
...
- Printed expectation checks for hypothetical profiles
- Printed survival predictions for hypothetical businesses
"""
...
def load_artifacts(
    ...
    with open(base / "logistic_pipeline.pkl", "rb") as file_obj:
        pipeline = pickle.load(file_obj)

    with open(base / "logistic_kept_columns.pkl", "rb") as file_obj:
        kept_columns = pickle.load(file_obj)

    with open(base / "logistic_evaluation_metrics.json", "r", encoding="utf-8") as file_obj:
...
def check_hypothetical_expectations(
    ...
    expectation_rows = [
        ...
        {
            "profile": "laundries",
            "feature": "business_category_laundries",
        },
        {
            "profile": "car_wash",
            "feature": "business_category_car_wash",
        },
        {
            "profile": "debt_collection",
            "feature": "business_category_debt_collection_agency",
        },
    ]
...
def main() -> None:
    """Entry point."""
    config = InspectConfig(
        artifacts_dir=Path("/content/drive/MyDrive/logreg_survival_outputs")
    )
